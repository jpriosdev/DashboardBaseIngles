...
const DAL = {
    async getFullQAData() {
      ...
      // Datos por sprint para test cases (filtrados por 'test' en resumen, excluyendo Bugs)
      const testCasesPerSprintRaw = await DAL.getTestCasesBySprint();
      const testCasesPerSprint = testCasesPerSprintRaw.map(s => ({
        sprint: s.sprint,
        sprint_num: s.sprint_num,
        testCases: s.total_tests || 0,
        testCasesExecuted: s.total_tests || 0
      }));
