...
async function getTestCasesBySprint() {
  return runQuery(`
    SELECT
      CAST(SUBSTR(sprint, -2) AS INTEGER) as sprint_num,
      sprint,
      -- Total test cases: cualquier registro con 'test' en el Resumen (sin importar tipo de incidencia)
      SUM(CASE WHEN LOWER(COALESCE(resumen,'')) LIKE '%test%' THEN 1 ELSE 0 END) as total_tests,
      -- Executed test cases: con 'test' en Resumen Y estado Done/Reviewed/Testing Complete/Approved for Release
      SUM(CASE 
        WHEN LOWER(COALESCE(resumen,'')) LIKE '%test%' 
...
async function getPlannedTestCasesBySprint() {
  return runQuery(`
    SELECT
      CAST(SUBSTR(sprint, -2) AS INTEGER) as sprint_num,
      sprint,
      -- Planned test cases: same as executed (todos los con 'test' en resumen)
      SUM(CASE WHEN LOWER(COALESCE(resumen,'')) LIKE '%test%' THEN 1 ELSE 0 END) as total_tests
    FROM bugs_detail
    GROUP BY sprint
    ORDER BY sprint_num
...
async function getCriticalBugsBySprintAndState() {
  ...
  return runQuery(`
    SELECT 
      sprint,
      SUM(CASE WHEN tipo_incidencia = 'Bug' AND prioridad = 'Major' AND estado = 'To Do' THEN 1 ELSE 0 END) as tareasPorHacer,
      SUM(CASE WHEN tipo_incidencia = 'Bug' AND prioridad = 'Major' AND estado = 'In Development' THEN 1 ELSE 0 END) as enProgreso,
      SUM(CASE WHEN tipo_incidencia = 'Bug' AND prioridad = 'Major' AND (estado = 'Ready for Testing' OR estado = 'Reopened') THEN 1 ELSE 0 END) as reabierto
    FROM bugs_detail
    WHERE tipo_incidencia = 'Bug' AND prioridad = 'Major'
    GROUP BY sprint
    ORDER BY sprint
  `);
}
