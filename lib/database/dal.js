...
async function getResolutionTimeAnalysis() {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // Buscar registros con Fail o Pass
      if (line.includes(',Fail,') || line.includes(',Pass,')) {
        ...
        // Extraer clave, estado, fecha, prioridad, Tag0 y Tag2 de la fila
        const parts = fullRow.split(',');
        const clave = parts[1]?.trim();
        const idIncidencia = parts[2]?.trim();      // ID de la incidencia
        const prioridad = parts[5]?.trim();          // Prioridad
        const tag0 = parts[11]?.trim();              // Tag0 (Sistema)
        const tag2 = parts[13]?.trim();              // Tag2 (Impacto)
        ...
        if (clave && estado) {
          if (!byKey[clave]) {
            byKey[clave] = { 
              ...
              failDates: [], 
              passDates: [],
              prioridad: prioridad || 'Unknown',
              tag0: tag0 || '',
              tag2: tag2 || '',
              ids: new Set()  // Mantener IDs únicos
            };
            totalUniqueTestCases++;
          }
          if (idIncidencia) {
            byKey[clave].ids.add(idIncidencia);
          }
          if (estado === 'Fail') {
            byKey[clave].fail++;
    ...
    Object.entries(byKey).forEach(([clave, data]) => {
      ...
      if (data.pass > 0 && data.failDates.length > 0 && data.passDates.length > 0) {
        ...
        unresolvedCasesDetail.push({ 
          ...
          tag0: data.tag0,
          tag2: data.tag2,
          idCount: data.ids.size  // Cantidad de IDs de incidencia únicos
        });
      }
    });
