...
async function getDeveloperModulesSummary() {
  return runQuery(`
    SELECT 
      ...
      COUNT(*) as count,
      SUM(CASE WHEN estado = 'To Do' THEN 1 ELSE 0 END) as pending,
      SUM(CASE WHEN estado != 'To Do' THEN 1 ELSE 0 END) as resolved
    FROM bugs_detail
...
async function getStatistics() {
  const [totalBugs, totalSprints, critical, pending, summary, bugStats, tcStats] = await Promise.all([
    ...
    runScalar("SELECT COUNT(*) as count FROM bugs_detail WHERE estado IN ('To Do', 'In Development') AND tipo_incidencia = 'Bug'"),
    getBugsSummary(),
    // vistas nuevas
